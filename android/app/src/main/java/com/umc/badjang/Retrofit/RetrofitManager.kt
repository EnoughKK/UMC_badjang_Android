...
//                Log.d(TAG, "RetrofitManager - onResponse() called / response : ${response.body()}")
    ...
    // 지원금 조회수 증가
    fun PatchSupportView(supportIdx: Long?, view: Int?, completion: (RESPONSE_STATE) -> Unit){

        val call = iSupportViewCount?.PatchSupportViewCount(supportIdx = supportIdx, view = view).let {
            it
        }?: return
    ...
    // 지원금 인덱스로 조회
    fun searchSupportIDx(supportIdx: Long?, completion: (RESPONSE_STATE, ArrayList<GetSupportDTO>?) -> Unit){
