...
class SchoolDeleteDialog (context:Context, post_idx : Int, schoolNameIdx:Int) : Dialog(context){
    ...
    val d_p_idx = post_idx
    val d_s_idx = schoolNameIdx
//    val post_idx = ApplicationClass.bSharedPreferences.getInt("post_idx", 0)
//    val schoolNameIdx = ApplicationClass.bSharedPreferences.getInt("schoolNameIdx", 0)
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.deleteOk.setOnClickListener {
            deleteSchoolPost(d_s_idx, d_p_idx)
        }
    ...
    private fun deleteSchoolPost(schoolNameIdx: Int, postIdx: Int){
        ...
        getOnePostInterface.deleteSchoolPost(schoolNameIdx, postIdx).enqueue(object :
            ...
            override fun onResponse(call: Call<PostDeletePostResponse>, response: Response<PostDeletePostResponse>) {
                if (response.isSuccessful) {
                    ...
                    if(result.message == "게시판을 삭제하였습니다."){
                        ApplicationClass.bSharedPreferences.edit().putString("delete_school_content","삭제").commit()
                        dismiss()
